...
export class LinkedInSmart {
  ...
  async init() {
    ...
    if (cookies) {
      await this.context.addCookies(cookies.map(c => ({ ...c, domain: c.domain?.startsWith('.') ? c.domain : (c.domain || '.linkedin.com') })))
    }
  ...
  async _extractProfileMetaFromCard(card) {
    // Works on "People you may know" cards on /mynetwork/
  ...
  async suggestedPeopleCanada(limit = 50) {
    ...
    for (let scroll=0; scroll<8 && out.length < limit; scroll++) {
      ...
      for (const card of cards) {
        ...
        // Optional preference for Open-to-work (not hard requirement)
        if (preferOTW && !meta.open_to_work) {
          // still allow, but we’ll soft-sort later
        }
